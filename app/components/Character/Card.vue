...
<template>
	<ClientOnly>
		<ContextMenu>
			<ContextMenuTrigger>
				<GlowBorderHover :color="['#A07CFE', '#FE8FB5', '#FFBE7B']" :border-radius="16" class="h-min w-min">
					<div class="Card-Container morph w-[256px] overflow-hidden border" :style="containerStyle">
						<div
							:class="
								cn('Card-Background h-[564px] w-[256px] bg-cover bg-center bg-no-repeat blur-xl', settingsStore.censorImages ? 'censor' : '')
							"
			...
			<ContextMenuContent>
				<ContextMenuItem @click="updateEvaluation">Refresh Evaluation</ContextMenuItem>
			</ContextMenuContent>
		</ContextMenu>
	</ClientOnly>
</template>
